...
function my_scripts()
{
  ...
  wp_enqueue_script('close-dialog', get_template_directory_uri() . '/scripts/close-dialog.js', array(), null, true);
...
function your_theme_category_class($the_list)
{
...
add_filter('the_category', 'your_theme_category_class');
    ...
    if (empty($_POST['user_nicename'])) {
      $errors->add(
        ...
        sprintf(
          ...
          esc_html__('Error'),
          esc_html__('Please enter a nice name.')
        ),
